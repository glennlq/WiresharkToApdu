...
import com.ecodigital.wiresharktoapdu.apdu.ApduStatusWord.ModeSW;
...
public class ApduListResponseISO7816 {    
    ...
    public ApduStatusWord searchResponse(ResponseAPDU responseAPDU) {
        ...
        String str = Hex.hexify((byte)responseAPDU.getSW1()) + Hex.hexify((byte)responseAPDU.getSW2());
    ...
    public ApduStatusWord searchResponse(byte SW1, byte SW2) {
        for(ApduStatusWord asw : listUnique){
            if( (SW1 & 0XFF) == asw.getIsw1() && (SW2 & 0XFF) == asw.getIsw2()) {
                return asw;
            }
        }
        
        ApduStatusWord tmp = null;
        String str = Hex.hexify(SW1) + Hex.hexify(SW2);
        for(ApduStatusWord asw : listGeneric) {
            String str2 = asw.getSw1() + asw.getSw2();
            boolean found = true;
            for(int i=0; i < 2; i++){
